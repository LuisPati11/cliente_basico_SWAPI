...
public class Main {	
    ...
    public static void masPoblado(SWClient sw) throws IOException{
    	// Obteniendo la cantidad de gente almacenada
    	int max_planet = sw.getNumberOfResources("planets");
    	if(max_planet == 0){
    		System.out.println("No se encontraron planetas.");
    	...
    	int contador = 0;
    	while(contador < 2){
    		Integer pl = getRandomResource(max_planet, used);
    		Planet planet = sw.getPlanet(sw.generateEndpoint("planets", pl));
    		
    		if(planet == null){
    			System.out.println("Hubo un error al encontrar el recurso " + pl);
    		} else {
    	...
    	Integer n = null;
    	do{
    		System.out.println("Cuál es el planeta más poblado? [0] "+ planets.get(0).name + " o [1] " + planets.get(1).name);
    		try{
    			n = Integer.parseInt(sc.nextLine());
    		}catch(NumberFormatException ex){
    			n = -1;
    	...
    	for(Planet pl: planets){
    		System.out.println(pl.name + " tiene una población de " + pl.population);
    	}
    	
    	// Resultado
    	if(Double.parseDouble(planets.get(n).population) >= Double.parseDouble(planets.get((n+1)%2).population)){
    		System.out.println("Enhorabuena!!! "+ acierto[rand.nextInt(acierto.length)]);
    	} else {
    		System.out.println("Fallaste :( " + fracaso[rand.nextInt(fracaso.length)]);
    	}
